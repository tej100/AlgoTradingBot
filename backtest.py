...
class WalkForwardBacktester:
    def __init__(self, ticker, interval, position='both'):
        self.ticker = ticker
    ...
    def download_data(self):
        ...
        try:
            data = yf.download(self.ticker, period="max", interval=self.interval, 
    ...
    def run_walk_forward_backtest(self, strategy_name, min_train_size=100, retrain_frequency=100):
        """
        Walk-forward backtesting with model retraining.
        For ML strategies (Ridge, RandomForest), this repeatedly calls model.fit()
        on expanding windows rather than fitting once on all data.
        """
        ...
        for i in range(start_idx, len(data)):
            if i == start_idx or (i - start_idx) % retrain_frequency == 0 or i == len(data) - 1:
                ...
                if (i - start_idx) % (retrain_frequency * 5) == 0 and i > start_idx:
                ...
                try:
                    ...
                    # Walk-forward: retrain/fit model on expanding window
                    result = strategy_method()  # This calls .fit() internally for ML strategies
...
if __name__ == "__main__":
    print("Starting Backtesting System...")
    print("="*100)
